...
Логический оператор AND всегда неявно присутствует, поэтому запрос "кошка собака" подразумевает, что должны быть найдены как "кошка", так и "собака". Явного оператора AND в системе не существует.
...
Логический оператор OR `|` имеет более высокий приоритет, чем AND, поэтому `поиск кошки | собаки | мыши` означает `поиск (кошки | собаки | мыши)`, а не `(поиск кошки) | собаки | мыши`. Оператор «или» задаётся в системе только прямой чертой `|`, в форме `OR` он не существует.
