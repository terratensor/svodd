...
namespace App\Suggestions\repositories;

use App\Suggestions\Entity\SearchQuery\SearchQuery;
use yii\data\BaseDataProvider;
use Manticoresearch\Search;

class SuggestionDataProvider extends BaseDataProvider
{
    ...
    protected function prepareModels(): array
    {
        ...
        if ($pagination === false) {
            ...
            foreach ($this->query->get() as $hit) {
                $id = $hit->getId();
                $comment = new SearchQuery($hit->getData());
            ...
            for ($count = 0; $count < $limit; ++$count) {
                ...
                $model = new SearchQuery($data->current()->getData());
